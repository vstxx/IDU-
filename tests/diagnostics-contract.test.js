...
(async () => {
  ...
  assert.equal(diagnostics.reportDateKey(new Date("2026-06-25T09:00:00.000Z")), "2026-06-25");
  ...
  const result = await diagnostics.reportActiveUser({
    ...
    runtime: { getManifest: () => ({ version: "0.3.2" }) },
    storage: storage.api,
  ...
  assert.equal(payload.extensionVersion, "0.3.2");
  ...
  assert.equal(fetchCalls.length, 1, "Same displayed name should report at most once per day");
  ...
  assert.equal(fetchCalls.length, 2, "Same displayed name should be reportable again on the next day");
  ...
  assert(fields.some((field) => field.name === "Extension version" && field.value === "0.3.2"));
